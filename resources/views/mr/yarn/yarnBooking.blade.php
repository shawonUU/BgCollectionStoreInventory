...
@section('js')
    ...
    <script>
         function changefabrication(id, changedColumn){
            ...
            if(!dzn) {$('#fabric'+id+'-cos_dzn').val(0); dzn=0;}
            pcsLoss = parseFloat($('#fabric'+id+'-process_loss').val());
            if(!pcsLoss) {$('#fabric'+id+'-process_loss').val(0); pcsLoss=0;}
            gsm = $('#fabric'+id+'-gsm').val();
            dia = $('#fabric'+id+'-dia').val();
            count = $('#fabric'+id+'-yarn_count').val();

            $.get('{{route('change_fabrication')}}',{id:id,fabric:fabric,item:item,fabricFor:fabricFor,dzn:dzn,gsm:gsm,dia:dia,count:count, pcsLoss:pcsLoss}, function(data){
        ...
        function getSelectedFabricItem(id, eleId ){
            ...
            for (var dx = 0; dx < comboChek.length; dx++) {
                if(comboChek[dx].id != eleId){comboChek[dx].checked = "";}
                else{
                    if(!comboChek[dx].checked){return;}
                }
            }

            $.get('{{route('get_selected_fabric_item')}}', {id:id}, function(res){
                let data = res[1];
        ...
        function setUnsetFabric(id){
            ...
            if(combo != null){
                ...
                $.get('{{route('set_unset_fabric')}}', {comboId:combo, fabricId:id, status:status}, function(data){
        ...
        function changeCombo(comboId, changedColumn, fabricId = null){
            ...
            let comboRow = [];
            $.get('{{route('get_selected_fabric_item')}}', {id:comboId}, function(res){
                let value = res[0];
                let data = res[1];
                if( changedColumn == 'qty' || changedColumn == 'extra_cutting' || changedColumn=='new_qty' || changedColumn=='rf' || changedColumn=='rg'){
                    for(let i=0; i<data.length; i++){
                        ...
                        if(changedColumn == 'rf' || changedColumn == 'rg'){
                            ...
                            $("#combo"+comboId+"-fabric"+data[i]+"-rg").css("background-color",highlighter);
                            $("#combo"+comboId+"-tf").css("background-color",highlighter);
                            $("#combo"+comboId+"-tg").css("background-color",highlighter);
                            $("#"+data[i]+"-tf").css("background-color",highlighter);
                            $("#"+data[i]+"-tg").css("background-color",highlighter);
                ...
                if(!comboRow[4]){$('#combo'+comboId+'-qty').val(0);comboRow[4] = 0;}
                comboRow[5] = $('#combo'+comboId+'-ecut').val();
                if(!comboRow[5]){$('#combo'+comboId+'-ecut').val(0);comboRow[5] = 0;}
                comboRow[6] = $('#combo'+comboId+'-nqty').val();
                if(!comboRow[6]){$('#combo'+comboId+'-nqty').val(0);comboRow[6] = 0;}
                comboRow[7] = $('#combo'+comboId+'-tf').val();
                if(!comboRow[7]){$('#combo'+comboId+'-tf').val(0);comboRow[7] = 0;}
                comboRow[8] = $('#combo'+comboId+'-tg').val();
                if(!comboRow[8]){$('#combo'+comboId+'-tg').val(0);comboRow[8] = 0;}
                ...
                $.ajax({
                    ...
                    type: 'post',
                    data: {orderId:'{{$orderId}}', comboId:comboId , changedColumn:changedColumn, comboRow:comboRow},
