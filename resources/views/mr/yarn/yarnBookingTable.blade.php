...
<div  class="d-flex justify-content-end  align-items-center">
    <div>
        <a href="{{route('print_booking_sheet', $orderId)}}" class="btn btn-sm btn-warning text-white shadow-none" target="_blank"><i class="fa fa-print"></i> Print</a>
    </div>
</div>
        ...
        <div style="padding-top: 100px !important; Width:{{$divWidth}}px !important;">
            <div style="margin-left: {{$margin}}px; display: inline-flex; position: relative;" >
                         ...
                         <div style="display:inline">
                             <table class="">
                                 <tr>
                                     ...
                                     <td><input style="border:none; width: 100%;" readonly  type="text"  value="{{ $orderInfo->buyer_name}}"></td>
                                 </tr>
                                 <tr>
                        ...
                        <div style="display:inline">
                            <table class="">
                                <tr>
                                    ...
                                    <td><input style="border:none" onchange="changeYarnBookingData({{ $orderId }})" type="date"  value="{{ $yarnBooking->issuing_date??"" }}" id="issuing_date"></td>
                                </tr>
                                <tr>
                                    <td class="px-2" >Shipment Date:</td>
                                    <td><input style="border:none" onchange="changeYarnBookingData({{ $orderId }})" type="date"  value="{{ $yarnBooking->shipment_date??"" }}" id="shipment_date"></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                                ...
                                @for($j=0; $j<$col; $j++)
                                     <td style="border: 1px solid #000; ">
                                         <div style="text-align: center; width: {{$width*2+1}}px !important;">
                                             <input onchange="setUnsetFabric({{$fabrications[$j]['id']}})" id="fabric{{$fabrications[$j]['id']}}-check" type="checkbox" class="checkBox table-input">
                                         </div>
                                     </td>
                                 @endfor
                            </tr>
                                    ...
                                    @if($j==0)
                                        <td>
                                            <div style="text-align: center; width: {{$width/2}}px !important;">
                                                <input value="{{$combos[$i]['id']}}" onchange="getSelectedFabricItem({{$combos[$i]['id']}}, 'combo{{$i}}-check')" id="combo{{$i}}-check" type="checkbox" class="table-input comboChek">
                                            </div>
                                        </td>
                                    @elseif($j<8 || $col2-4<$j)
                                        @php
                                        ...
                                        <td>
                                            <div style="text-align: center; width: {{$j==2? $width*2:$width}}px !important;">
                                                <input onchange="changeCombo({{$combos[$i]['id']}}, '{{$comboColumn}}')" id="{{$comboId}}" type="{{$type}}" value="{{floatFormater($combos[$i][$comboColumn])}}" class="table-input" {{$readonly?'readonly':''}}>
                                            </div>
                                        </td>
                                    @else
                                        @php
                                            ...
                                            if(isset($yarnAllocations[$allocationId]) && $yarnAllocations[$allocationId]['fabric_id'] == $fabrications[$fabricDx]['id']){
                                                ...
                                                if($j%2==1) {$yarnAllocationsColumnValue = $yarnAllocations[$allocationId]['req_gray'];$allocationId++;}
                                                $readonly = null;
                                            }
                                            if($j%2==1) $fabricDx++;
                                        @endphp
                                        <td>
                                            <div style="text-align: center; width: {{$width}}px !important;">
                                                <input onchange="changeCombo({{$combos[$i]['id']}}, '{{$j%2==0?'rf':'rg'}}', {{$fabricId}})" id="{{$comboId}}" type="number" value="{{floatFormater($yarnAllocationsColumnValue)}}" class="table-input" {{$readonly??''}}>
                                            </div>
                                        </td>
                                    @endif
                                @endfor
                            ...
                            @for ($j=0; $j<$col2-7; $j++)
                                ...
                                @elseif($j<$col2-10)
                                    @php
                                        ...
                                        if($j%2==1){$id = $fabrications[$fabricCol]['id']."-tf"; $vlo = $fabrications[$fabricCol]['total_finished']; $tf += $vlo;}
                                        if($j%2==0){$id = $fabrications[$fabricCol]['id']."-tg"; $vlo = $fabrications[$fabricCol++]['total_gray']; $tg +=$vlo;}
                                    @endphp
                                    <td >
                                        <div style="text-align: center; width: {{$width}}px !important;">
                                            <input id="{{$id}}" type="number" value="{{floatFormater($vlo)}}" class="table-input" readonly>
                                        </div>
                                    </td>
                                @elseif($j<$col2-8)
                                    @php
                                        if($j%2==1){$id = 'tf'; $vlo = $tf;}
                                        if($j%2==0){$id = 'tg'; $vlo = $tg;}
                                    @endphp
                                    <td>
                                        <div style="text-align: center; width: {{$width}}px !important;">
                                            <input id="{{$id}}" type="number" value="{{floatFormater($vlo)}}" class="table-input" readonly>
                ...
                <div>
                    ...
                    <p class="d-inline" style="font-size: 10px; padding: 0px; color: black; margin-top: 5px;">Booking Based On Knitting & Dyeing Process Loss
                        <span>
                            <input onchange="changeYarnBookingData({{ $orderId }}, 'processLose')" type="number" min="0" value="{{ floatFormater($yarnBooking->process_loss??"") }}" id="process_loss" style="width: 30px; margin-left: 5px;font-size: 12px; font-weight: bold; text-align: center;">
                            %
                        </span>
                        Extra Cutting-
                        <span>
                            <input onchange="changeYarnBookingData({{ $orderId }}, 'processLose')" type="number" min="0" value="{{ floatFormater($yarnBooking->extra_cutting??"") }}" id="extra_cutting" style="width: 30px; margin-left: 5px;font-size: 12px; font-weight: bold; text-align: center;">
                            %
                        </span>
                    </p>
